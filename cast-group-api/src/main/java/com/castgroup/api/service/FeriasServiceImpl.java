...
public class FeriasServiceImpl implements FeriasService {
	...
	public void checkPeriodoFerias(List<Funcionario> equipeFuncionarios, Ferias ferias)
			throws Exception {
			...
			for (Funcionario funcionario: equipeFuncionarios) {
				if (funcionario.getId() != ferias.getFuncionario().getId()
					...
					if (ultDtFerias.isBefore(dtferias)) {
						...
						throw new Exception("Equipes de até 4 funcionários não podem tirar férias em período coincidente.");
					}
				}
			}
		}
